...
if __name__ == '__main__':
    ...
    # Compute the similarity matrix (kernel)
    similarity_matrix = util.cos_sim(embeddings, embeddings)  # Shape: [N, N]

    # Convert the similarity matrix to a list of lists for JSON serialization
    kernel = similarity_matrix.cpu().tolist()

    # Save the kernel matrix to a JSON file
    with open('dummy_training_data/sentence_kernel_matrix.json', 'w') as f:
        json.dump(kernel, f, indent=2)

    # Optionally, save the kernel matrix to a CSV file
    with open('dummy_training_data/sentence_kernel_matrix.csv', 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow([''] + [f"Sentence {i}" for i in range(len(sentences))])
        for i, row in enumerate(kernel):
            writer.writerow([f"Sentence {i}"] + row)
